...
const KASANE_STABLE = getChainInfo(UniverseChainId.Kasane).tokens.stablecoins[0]
...
test.describe(
  ...
  () => {
    test.describe('URL state parsing and persistence', () => {
      test.describe('Backwards compatibility', () => {
        test('feeTier and isDynamic', async ({ page }) => {
          test.skip(true, 'Create position flow is v2-only in this build')
      ...
      test('parses simple query params and resets', async ({ page }) => {
        await page.goto(
          buildUrl({
            subPath: '/v2',
            queryParams: {
              currencyB: KASANE_STABLE.address,
            },
          }),
        )
        // Should default to native token when currencyA is missing
        await expect(page.getByRole('button', { name: 'ICP' })).toBeVisible()
        await expect(page.getByRole('button', { name: KASANE_STABLE.symbol })).toBeVisible()
    ...
    test.describe.skip('Token sorting', () => {
      test.describe('V4', () => {
    ...
    test.describe.skip('Price range', () => {
