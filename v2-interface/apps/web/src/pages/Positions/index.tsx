...
import { useSupabaseUserV2PositionsQuery } from 'state/positions/useSupabaseUserV2PositionsQuery'
...
export default function PositionsPage() {
  ...
  const { data, isLoading, isError, error, refetch, isFetching } = useSupabaseExploreStatsQuery({
  ...
  return (
    <Flex gap="$gap16" px="$spacing16" py="$spacing16" width="100%" maxWidth={960} alignSelf="center">
      <Flex row justifyContent="space-between" alignItems="center" flexWrap="wrap" gap="$gap8">
        <Flex gap="$gap4">
          ...
          <Text variant="body2" color="$neutral2">
            Kasane LPプール一覧（Supabase）
          </Text>
        </Flex>

        <Flex row gap="$gap8">
          <Button variant="default" size="small" onPress={() => refetch()} isDisabled={isFetching}>
            再読み込み
          </Button>
          <Button
            variant="default"
      ...
      <Flex gap="$gap8" borderColor="$surface3" borderWidth="$spacing1" borderRadius="$rounded16" p="$padding12">
        <Text variant="subheading2">あなたのV2 LPポジション</Text>
        {!walletAddress ? (
