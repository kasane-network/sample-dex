...
function getErrorContent({
  ...
  chainId: ReturnType<typeof toSupportedChainId>
}): {
  title: JSX.Element
  message?: JSX.Element
  ...
  switch (errorType) {
    ...
    case PendingModalError.WRAP_ERROR:
      return {
        ...
        message: <>{formatWrapMessageByChain(t('token.wrap.fail.message'), chainId ?? undefined)}</>,
...
export default function Error({ errorType, trade, showTrade, swapResult, onRetry }: ErrorModalContentProps) {
  ...
  const chainId = toSupportedChainId(trade?.inputAmount.currency.chainId ?? accountChainId)
  const { title, message, supportArticleURL } = getErrorContent({ errorType, trade, t, chainId })
