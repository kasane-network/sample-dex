...
import { PoolSortFields, TablePool } from 'dataLayer/data/pools/useTopPools'
import { gqlToCurrency, OrderDirection, unwrapToken } from 'dataLayer/data/util'
...
import { ExploreTokenRef, PoolStat } from 'state/explore/types'
  ...
  const poolTableValues: PoolTableValues[] | undefined = useMemo(
    () =>
      pools?.map((pool, index) => {
        ...
        const chainId = pool.token0?.chain ? (fromGraphQLChain(pool.token0.chain) ?? defaultChainId) : defaultChainId
