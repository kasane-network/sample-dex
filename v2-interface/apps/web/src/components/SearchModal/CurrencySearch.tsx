  ...
  const handleCurrencySelectTokenSelectorCallback = useCallback(
    async ({ currency }: { currency: Currency }) => {
      if (!isMultichainContext) {
        ...
        // In this Kasane-focused fork, allow selecting Kasane assets even if the wallet
        // cannot switch chains from the selector flow. Chain mismatch is still handled later.
        if (!correctChain && currency.chainId !== UniverseChainId.Kasane) {
          return
        }
      }
