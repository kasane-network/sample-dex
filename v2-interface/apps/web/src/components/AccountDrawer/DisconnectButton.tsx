...
function DisconnectTraceWrapper({ children }: PropsWithChildren) {
  ...
  return (
    <Trace logPress element={ElementName.DisconnectWalletButton} properties={{ connector_id: evmConnectorId }}>
      {children}
    </Trace>
  )
}
...
function getSwitchButtonText({
  ...
  const switchPlatformText = t('common.connectAWallet.button.evm.switch')
  const connectPlatformText = t('common.connectAWallet.button.evm')

  switch (variant) {
    case 'switch':
      return t('common.connectAWallet.button.switch')
    case 'switch_platform':
      return switchPlatformText
    case 'connect_platform':
      return connectPlatformText
    default:
...
function useSwitchButtonVariants() {
  const isEVMConnected = useAccountsStore((state) => state.getConnectionStatus(Platform.EVM).isConnected)

  return useMemo(() => {
    return [
      {
        platform: Platform.EVM,
        variant: getSwitchButtonVariant({ connectedOnThisPlatform: isEVMConnected, connectedOnOtherPlatform: false }),
      },
    ]
  }, [isEVMConnected])
}
...
function SwitchWalletButtonRow({ variant, platform }: { variant: SwitchButtonVariant; platform: Platform }) {
  ...
  const text = getSwitchButtonText({ t, variant })
