...
export async function transactionToActivity({
  ...
  details?: InterfaceTransactionDetails | null
  formatNumber: FormatNumberFunctionType
...
export function getTransactionToActivityQueryOptions({
  ...
  transaction?: InterfaceTransactionDetails | null
  formatNumber: FormatNumberFunctionType
}) {
  return queryOptions({
    queryKey: [ReactQueryCacheKey.TransactionToActivity, transaction],
    queryFn: transaction ? async () => transactionToActivity({ details: transaction, formatNumber }) : skipToken,
  })
}
...
export function getFORTransactionToActivityQueryOptions({
  ...
  transaction?: FORTransaction | null
  formatNumber: FormatNumberFunctionType
  formatFiatPrice: FormatFiatPriceFunctionType
}) {
  return queryOptions({
    queryKey: [ReactQueryCacheKey.TransactionToActivity, transaction],
    queryFn: transaction ? async () => forTransactionToActivity({ transaction, formatNumber, formatFiatPrice }) : skipToken,
  })
}
...
export function useLocalActivities(account: string): ActivityMap {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      return createActivityMapByHash(allActivities.map((activity) => activity ?? undefined))
    },
  })
