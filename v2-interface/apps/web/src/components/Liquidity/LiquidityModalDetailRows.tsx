...
export function LiquidityModalDetailRows({
  ...
  const symbol0 = currency0Amount?.currency.symbol
  const symbol1 = currency1Amount?.currency.symbol
    ...
    <Flex px="$padding16" gap="$gap8">
        ...
        LineItem={{
          Label: () => (
            <Text variant="body3" color="$neutral2">
              <Trans i18nKey={symbol0 ? 'pool.specificPosition' : 'pool.position'} values={symbol0 ? { symbol: symbol0 } : undefined} />
            </Text>
          ),
          Value: () => (
      ...
      <DetailLineItem
        LineItem={{
          Label: () => (
            <Text variant="body3" color="$neutral2">
              <Trans i18nKey={symbol1 ? 'pool.specificPosition' : 'pool.position'} values={symbol1 ? { symbol: symbol1 } : undefined} />
            </Text>
          ),
          Value: () => (
            <Flex row gap="$gap4" alignItems="center">
