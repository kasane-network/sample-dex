...
describe('useUpdatedAmountsFromDependentAmount', () => {
  ...
  it('does not override values when dependentAmount0 is 0', () => {
    const { result } = renderHook(() =>
      useUpdatedAmountsFromDependentAmount({
        ...baseProps,
        dependentAmount: '0',
    ...
    expect(result.current.updatedFormattedAmounts?.[PositionField.TOKEN0]).toBe(token0FormattedAmount)
    expect(result.current.updatedDeposit0Disabled).toBe(false)
    expect(result.current.updatedDeposit1Disabled).toBe(false)
  })

  it('does not override values when dependentAmount1 is 0', () => {
    const { result } = renderHook(() =>
      useUpdatedAmountsFromDependentAmount({
        ...baseProps,
        dependentAmount: '0',
    ...
    expect(result.current.updatedFormattedAmounts?.[PositionField.TOKEN1]).toBe(token1FormattedAmount)
    expect(result.current.updatedDeposit0Disabled).toBe(false)
    expect(result.current.updatedDeposit1Disabled).toBe(false)
  })
