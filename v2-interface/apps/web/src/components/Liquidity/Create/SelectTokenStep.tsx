...
export function SelectTokensStep({
  ...
  const continueButtonEnabled =
    creatingPoolOrPair || poolOrPair || (protocolVersion === ProtocolVersion.V2 && Boolean(token0 && token1))
  ...
  const supportedChains = useMemo(() => {
    return protocolVersion === ProtocolVersion.V4 ? chains.filter((chain) => !isV4UnsupportedChain(chain)) : undefined
  }, [protocolVersion, chains])

  const openCurrencySearch = useCallback((field: 'tokenA' | 'tokenB') => {
    setTimeout(() => {
      setCurrencySearchInputState(field)
    }, 0)
  }, [])

  const handleOnContinue = () => {
    // v2 flow has no hook/wrapped-native gate. Move directly to next step.
  ...
  const isContinueDisabled =
    protocolVersion === ProtocolVersion.V2
      ? !token0 || !token1
      : !continueButtonEnabled || hasError || (showWrappedNativeWarning && !!wrappedNativeWarning)
        ...
        <Flex gap="$spacing32" {...rest}>
                ...
                <Flex row gap="$gap16" $md={{ flexDirection: 'column' }}>
                  <Flex row flex={1} flexBasis={0} $md={{ flexBasis: 'auto' }}>
                    <CurrencySelector
                      ...
                      onPress={() => openCurrencySearch('tokenA')}
                    />
                  </Flex>
                  <Flex row flex={1} flexBasis={0} $md={{ flexBasis: 'auto' }}>
                    <CurrencySelector
                      ...
                      onPress={() => openCurrencySearch('tokenB')}
                    />
                  </Flex>
                </Flex>
              )}
          ...
          <Flex row>
            <Button
              ...
              loading={protocolVersion === ProtocolVersion.V2 ? false : Boolean(!continueButtonEnabled && token0 && token1 && fee)}
