...
export function useDepositInfo(state: UseDepositInfoProps): DepositInfo {
  ...
  useEffect(() => {
    ...
    const v2PairState =
      protocolVersion === ProtocolVersion.V2 && state.poolOrPair instanceof Pair
        ? {
            reserve0: state.poolOrPair.reserve0.toExact(),
            reserve1: state.poolOrPair.reserve1.toExact(),
            token0: state.poolOrPair.token0.address,
            token1: state.poolOrPair.token1.address,
          }
        : undefined

    console.info('[LP_DEBUG][useDepositInfo]', {
      protocolVersion,
      exactField,
      exactAmounts,
      inputSymbol: independentToken?.symbol,
      inputAmount: independentAmount?.toExact(),
      dependentSymbol: dependentToken?.symbol,
      dependentAmount: dependentAmount?.toExact(),
      parsedToken0: parsedAmounts[PositionField.TOKEN0]?.toExact(),
      parsedToken1: parsedAmounts[PositionField.TOKEN1]?.toExact(),
      skipDependentAmount: state.skipDependentAmount ?? false,
      v2PairState,
    })
  }, [
    dependentAmount,
