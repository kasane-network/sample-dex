...
export function getInitialLogoUrl({
  ...
  const defaultChainInfo = getChainInfo(DEFAULT_CHAIN_ID)
  const networkName = isUniverseChainId(chainId)
    ? (getChainInfo(chainId).assetRepoNetworkName ?? defaultChainInfo.assetRepoNetworkName ?? 'ethereum')
    : (defaultChainInfo.assetRepoNetworkName ?? 'ethereum')
  const checksummedAddress = getValidAddress({
    address,
    chainId: isUniverseChainId(chainId) ? chainId : DEFAULT_CHAIN_ID,
