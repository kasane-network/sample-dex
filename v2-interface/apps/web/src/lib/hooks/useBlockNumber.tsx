...
export function BlockNumberProvider({ children }: PropsWithChildren) {
    ...
    setChainBlock((chainBlock) => {
      if (chainBlock.chainId === chainId) {
        if (!chainBlock.block || chainBlock.block < block) {
          const mainnetBlock = chainId === UniverseChainId.Kasane ? block : chainBlock.mainnetBlock
          return { chainId, block, mainnetBlock }
        }
      } else if (chainId === UniverseChainId.Kasane) {
  ...
  useEffect(() => {
    RPC_PROVIDERS[UniverseChainId.Kasane]
      .getBlockNumber()
      .then((block) => onChainBlock(UniverseChainId.Kasane, block))
