...
import {
  consumeSwapSagaParams,
  registerSwapSagaParams,
  type SwapSagaRuntimePayload,
} from 'state/sagas/transactions/runtimeParamsRegistry'
...
import {
  registerPlanSagaParams,
  type PlanSagaRuntimePayload,
} from 'uniswap/src/features/transactions/swap/plan/runtimeParamsRegistry'
...
import { BridgeTrade, ChainedActionTrade, ClassicTrade, KasaneV2Trade } from 'uniswap/src/features/transactions/swap/types/trade'
...
function* handleSwapTransactionStep(params: HandleSwapStepParams): SagaGenerator<string> {
  ...
  if (trade instanceof KasaneV2Trade) {
    ...
    handleSwapTransactionAnalytics({ trade, analytics, hash })
  }
...
export type SwapParams = SwapExecutionCallbacks & {
...
function* swap(params: SwapParams) {
  ...
  setSteps(steps)
  if (!steps.length) {
    const error = new UnexpectedTransactionStateError('No transaction steps generated for swap execution')
    logger.error(error, {
      tags: { file: 'swapSaga', function: 'swap/generateSwapTransactionSteps' },
      extra: {
    ...
    for (step of steps) {
      switch (step.type) {
        ...
        case TransactionStepType.SwapTransactionAsync: {
          ...
          yield* handleSwapTransactionStep({
            ...
            allowDuplicativeTx: trade instanceof KasaneV2Trade,
          })
          break
        }
        case TransactionStepType.SwapTransactionBatched: {
          requireRouting(trade, [TradingApi.Routing.CLASSIC, TradingApi.Routing.BRIDGE])
          if (trade instanceof KasaneV2Trade) {
            throw new UnexpectedTransactionStateError('Kasane trade does not support batched swap execution')
          }
          yield* handleSwapTransactionBatchedStep({
            address: account.address,
            step,
            setCurrentStep,
            trade,
...
function* swapFromRuntimePayload(params: SwapSagaRuntimePayload) {
  const swapParams = consumeSwapSagaParams(params.runtimeId)
  if (!swapParams) {
    logger.warn('swapSaga', 'swapFromRuntimePayload', 'Missing runtime params for swap saga', {
      runtimeId: params.runtimeId,
    })
    return
  }
  yield* call(swap, swapParams)
}

export const swapSaga = createSaga(swapFromRuntimePayload, 'swapSaga')
  ...
  return useCallback(
    (args: SwapCallbackParams) => {
      ...
      if (!account || !isSignerMnemonicAccountDetails(account)) {
        logger.warn('swapSaga', 'useSwapCallback', 'Missing signer account for swap submission', {
      ...
      if (swapTxContext.trade.routing === TradingApi.Routing.CHAINED) {
        ...
        const planRuntimePayload: PlanSagaRuntimePayload = registerPlanSagaParams(planParams)
        appDispatch(
          planSaga.actions.trigger(planRuntimePayload),
        )
      } else {
        const swapRuntimePayload = registerSwapSagaParams(swapParams)
        appDispatch(swapSaga.actions.trigger(swapRuntimePayload))
      }
