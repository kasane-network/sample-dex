import { SparklineMap } from 'dataLayer/data/types'
import { PricePoint, TimePeriod, unwrapToken } from 'dataLayer/data/util'
import {
...
import { Amount, PriceHistory, TokenStat } from 'state/explore/types'
...
function convertTokenStatsToTokenStat(tokenStats: TokenStat, duration: TimePeriod): TokenStat {
...
export function useTopTokens() {
  ...
  const tokenStats = data?.stats?.tokenStats.map((tokenStat) =>
  ...
  const tokenSortRank = useMemo(
    () =>
      ...
      sortedTokenStats?.reduce((acc, cur, i) => {
        ...
        const currCurrencyId = buildCurrencyId(fromGraphQLChain(cur.chain) ?? DEFAULT_CHAIN_ID, cur.address)
        return {
          ...acc,
          [currCurrencyId]: i + 1,
        }
  ...
  const sparklines = useMemo(() => {
    const unwrappedTokens = filteredTokens?.map((tokenStat) => {
      const chainId = fromGraphQLChain(tokenStat.chain) ?? getChainIdFromChainUrlParam(tokenStat.chain.toLowerCase())
