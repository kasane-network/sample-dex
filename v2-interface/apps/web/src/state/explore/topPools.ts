import {
  ...
  PoolSortFields,
  PoolTableSortState,
} from 'dataLayer/data/pools/useTopPools'
...
import { ExploreStatsData, PoolStat } from 'state/explore/types'
...
function convertPoolStatsToPoolStat(poolStats: PoolStat): PoolStat {
  return {
    ...poolStats,
    apr: calculateApr({
      ...
      feeTier: poolStats.feeTier?.feeAmount ?? V2_DEFAULT_FEE_TIER,
    }),
    boostedApr: poolStats.boostedApr,
    feeTier: {
      feeAmount: poolStats.feeTier?.feeAmount ?? V2_DEFAULT_FEE_TIER,
...
function getPoolDataByProtocol(
  data: ExploreStatsData | undefined,
