...
function useCurrencyPreprocessing({
  ...
  const nativeAddressWithFallback = getChainInfo(supportedChainId ?? DEFAULT_CHAIN_ID).nativeCurrency.address
  ...
  const currencyId = buildCurrencyId(supportedChainId ?? DEFAULT_CHAIN_ID, addressWithFallback)
...
export function useToken(tokenAddress?: string, chainId?: UniverseChainId): Maybe<Token> {
  ...
  const formattedAddress = getValidAddress({
    ...
    chainId: chainId ?? connectedChainId ?? DEFAULT_CHAIN_ID,
