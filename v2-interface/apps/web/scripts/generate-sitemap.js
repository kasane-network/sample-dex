...
if (process.env.ENABLE_EXTERNAL_UNISWAP_FETCH !== 'true') {
  console.log('Skipping external Uniswap API calls for sitemap generation')
  process.exit(0)
}
...
fs.readFile('./public/tokens-sitemap.xml', 'utf8', async (_err, data) => {
  ...
  try {
    ...
    const tokensResponse = await fetch(
      '/__disabled_api__/v2/uniswap.explore.v1.ExploreStatsService/TokenRankings?connect=v1&encoding=json&message=' +
...
fs.readFile('./public/pools-sitemap.xml', 'utf8', async (_err, data) => {
  ...
  try {
    ...
    for (const chainName of chains) {
      const poolsResponse = await fetch('/__disabled_api__/v1/graphql', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Origin: 'https://app.uniswap.org',
