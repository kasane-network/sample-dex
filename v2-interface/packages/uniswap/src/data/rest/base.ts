...
function createDisabledTransport(baseUrl: string): Transport {
  const createDisabledError = (methodName: string): Error =>
    new Error(`External Connect transport is disabled: ${methodName} ${baseUrl}`)

  return {
    unary: async () => {
      throw createDisabledError('unary')
    },
    stream: async () => {
      throw createDisabledError('stream')
    },
  }
}

export const createConnectTransportWithDefaults = (options: Partial<ConnectTransportOptions> = {}): Transport => {
  if (uniswapUrls.apiBaseUrlV2.startsWith('/__disabled_api__')) {
    return createDisabledTransport(uniswapUrls.apiBaseUrlV2)
  }
