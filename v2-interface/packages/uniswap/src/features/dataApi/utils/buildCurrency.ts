...
export function buildCurrency(args: BuildCurrencyParams): Token | NativeCurrency | undefined {
  ...
  if (chainId === UniverseChainId.Solana && address) {
    ...
    if (!isNonNativeAddress(chainId, address)) {
      ...
      const isWrappedNativeAddress =
        wrappedNative &&
        areAddressesEqual({
          addressInput1: { address, chainId },
          addressInput2: { address: wrappedNative.address, chainId },
        })

      // If backend labels wrapped native as WETH on Kasane, keep UI symbol/name from chain config (WICP).
      result = isWrappedNativeAddress
        ? wrappedNative
