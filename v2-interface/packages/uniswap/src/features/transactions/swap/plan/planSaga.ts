...
function* planFromRuntimePayload(params: PlanSagaRuntimePayload) {
  const planParams = consumePlanSagaParams(params.runtimeId)
  if (!planParams) {
    logger.warn('planSaga', 'planFromRuntimePayload', 'Missing runtime params for plan saga', {
      runtimeId: params.runtimeId,
    })
    return
  }
  yield* call(plan, planParams)
}

export const planSaga = createSaga(planFromRuntimePayload, 'planSaga')
