...
export type ValidatedIndicativeQuoteResponse = TradingApi.QuoteResponse & {
  ...
  quote: ClassicQuoteResponse['quote'] & {
...
export function validateIndicativeQuoteResponse(
  ...
  if (!isClassicQuoteResponse(response)) {
    return undefined
  }
...
function isClassicQuoteResponse(response: TradingApi.QuoteResponse): response is ClassicQuoteResponse {
  return response.routing === TradingApi.Routing.CLASSIC
}
...
export class KasaneV2Trade {
  readonly routing = TradingApi.Routing.CLASSIC
  readonly quote: ValidatedIndicativeQuoteResponse
  ...
  readonly slippageTolerance: number = 0.5
  ...
  constructor({
    ...
    this.quote = quote
    this.tradeType = tradeType
    this.routePath = routePath ?? [quote.input.token, quote.output.token]

    const inputAmount = getCurrencyAmount({
      value: quote.input.amount,
      valueType: ValueType.Raw,
      currency: currencyIn,
    })
    const outputAmount = getCurrencyAmount({
    ...
    if (!inputAmount || !outputAmount) {
      throw new Error('Error parsing KasaneV2Trade currency amounts')
    }

    this.inputAmount = inputAmount
    this.outputAmount = outputAmount
    this.executionPrice = new Price(currencyIn, currencyOut, quote.input.amount, quote.output.amount)
