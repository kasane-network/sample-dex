...
const SWAP_SUCCESS_DISPLAY_MS = 1200
...
export function useCreateSwapReviewCallbacks(ctx: {
  ...
  const onSuccess = useCallback(() => {
    ...
    if (isWebApp) {
      ...
      setTimeout(
        ...
        Math.max(SWAP_SUCCESS_DISPLAY_MS, ADAPTIVE_MODAL_ANIMATION_DURATION),
      )
      return
    }
    onClose()
  ...
  const executeSwap = useEvent(async () => {
    ...
    const isKasaneTrade = swapParams.trade.inputAmount.currency.chainId === UniverseChainId.Kasane
    if (isKasaneTrade) {
      const { data: kasaneFreshSwapTxData, error: kasaneSwapTxError } = await tryCatch(
        swapTxAndGasInfoService.getSwapTxAndGasInfo({
      ...
      if (kasaneSwapTxError || !kasaneFreshSwapTxData) {
        const wrappedError = new Error('Failed to build Kasane swap transaction before wallet request', {
          cause: kasaneSwapTxError,
        })
        logger.error(wrappedError, {
          tags: { file: 'useCreateSwapReviewCallbacks.ts', function: 'executeSwap/kasaneFreshTx' },
          extra: {
            tradeRouting: swapParams.trade.routing,
            chainId: swapParams.trade.inputAmount.currency.chainId,
          },
        })
      ...
      const executeSwapService = getExecuteSwapService({
        ...
        setCurrentStep,
        setSteps,
        getSwapTxContext: () => kasaneFreshSwapTxData,
      })
  ...
  const submitTransaction = useEvent(async () => {
    if (reviewScreenWarning && !showWarningModal && !warningAcknowledged) {
      logger.warn(
        ...
        'submitTransaction',
        'swap submit blocked by warning acknowledgement gate',
        {
          warningType: reviewScreenWarning.warning.type,
    ...
    try {
      ...
      onFailure(error instanceof Error ? error : new Error('executeSwap failed'))
    }
  })
