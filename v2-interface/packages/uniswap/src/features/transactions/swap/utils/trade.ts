...
function hasProtocolRoutes(trade: Trade): trade is Extract<Trade, { routes: { protocol: Protocol }[] }> {
  return 'routes' in trade && Array.isArray(trade.routes)
}

export function getProtocolVersionFromTrade(trade: Trade): Protocol | undefined {
  if (!isClassic(trade) || !hasProtocolRoutes(trade)) {
    return undefined
  }
