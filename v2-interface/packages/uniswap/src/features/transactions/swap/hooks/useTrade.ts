...
function parseTradeResult(input: {
  ...
  const isKasaneTrade =
    currencyIn?.chainId === UniverseChainId.Kasane && currencyOut?.chainId === UniverseChainId.Kasane
  ...
  if (!data?.trade || !currencyIn || !currencyOut) {
    return {
      ...
      isFetching,
      trade: null,
      indicativeTrade: isLoading || isKasaneTrade ? indicative.trade : undefined,
