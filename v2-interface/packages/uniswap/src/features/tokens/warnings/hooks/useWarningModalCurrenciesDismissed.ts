...
export function useWarningModalCurrenciesDismissed({
  ...
  const address0 = t0.currencyId ? currencyIdToAddress(t0.currencyId) : undefined
  ...
  const { tokenWarningDismissed: tokenWarningDismissed0, onDismissTokenWarning: onDismissTokenWarning0 } =
    useDismissedTokenWarnings(
      t0.currency.isNative || !address0 ? undefined : { chainId: t0.currency.chainId, address: address0 },
      tokenProtectionWarning0,
    )

  const address1 = t1?.currencyId ? currencyIdToAddress(t1.currencyId) : undefined
