...
function getAddressFromCurrencyInfo(currencyInfo: CurrencyInfo): string | undefined {
  if (!currencyInfo.currencyId) {
    return undefined
  }
  try {
    return currencyIdToAddress(currencyInfo.currencyId)
  } catch {
    return undefined
  }
}
...
function TokenWarningModalContent({
  ...
  const sendReport = useEvent((reportText: string) => {
    const tokenAddress = getAddressFromCurrencyInfo(currencyInfo0)
    if (!tokenAddress) {
      return
    }
  ...
  const analyticsProperties = {
    tokenSymbol,
    tokenAddress: getAddressFromCurrencyInfo(currencyInfo0),
    chainId: currencyInfo0.currency.chainId,
    // if both tokens are low or blocked severities, their warnings are combined into 1 plural screen
    tokenSymbol1: currencyInfo1?.currency.symbol,
    tokenAddress1: currencyInfo1 ? getAddressFromCurrencyInfo(currencyInfo1) : undefined,
