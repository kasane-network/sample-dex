...
export default function parseTradeTransaction(
  ...
  const sent = txAssetChanges.find((t) => t.direction === BackendApi.TransactionDirection.Out)
  ...
  const { received, refund } = txAssetChanges.reduce<{
    ...
    (acc, t) => {
      if (t.direction !== BackendApi.TransactionDirection.In) {
        return acc
      }
      ...
      const isRefundInternalTx =
        ...
        t.tokenStandard === BackendApi.TokenStandard.Native
  ...
  if (onlyERC20Tokens) {
    const inputCurrencyId =
      sent.tokenStandard === BackendApi.TokenStandard.Native
        ? buildNativeCurrencyId(chainId)
        : sent.asset.address
          ? buildCurrencyId(chainId, sent.asset.address)
          : null
    const outputCurrencyId =
      received.tokenStandard === BackendApi.TokenStandard.Native
        ? buildNativeCurrencyId(chainId)
  ...
  if (containsNFT) {
    ...
    const purchaseCurrencyId =
      tokenChange.tokenStandard === BackendApi.TokenStandard.Native
        ? buildNativeCurrencyId(chainId)
