...
export const COMMON_BASES: ChainCurrencyList = {
  ...
  [UniverseChainId.Kasane]: [
    nativeOnChain(UniverseChainId.Kasane),
    WRAPPED_NATIVE_CURRENCY[UniverseChainId.Kasane] as Token,
    TEST_ETH_KASANE,
    USDC_KASANE,
  ].map(buildPartialCurrencyInfo),
...
function getTokenLogoURI(chainId: UniverseChainId, address: string): ImageSourcePropType | string | undefined {
  ...
  if (chainId === UniverseChainId.Kasane) {
    const wrappedKasaneCurrency = WRAPPED_NATIVE_CURRENCY[UniverseChainId.Kasane]
    if (
      wrappedKasaneCurrency &&
      areAddressesEqual({
        addressInput1: { address, platform: Platform.EVM },
        addressInput2: { address: wrappedKasaneCurrency.address, platform: Platform.EVM },
      })
    ) {
      return ICP_LOGO as ImageSourcePropType
    }
  }
