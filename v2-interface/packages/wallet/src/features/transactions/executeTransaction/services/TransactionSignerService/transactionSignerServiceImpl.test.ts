...
describe('TransactionSignerService', () => {
  ...
  beforeEach(() => {
    ...
    mockGetGasPrice.mockResolvedValue(BigNumber.from('123'))
  })
  ...
  describe('prepareTransaction', () => {
    ...
    it('should force EIP-1559 gas fields for Kasane transactions before populate', async () => {
      // Arrange
      const service = createTestService()
      ...
      const populatedRequest = {
        ...request,
        type: 2,
        maxFeePerGas: BigNumber.from('123'),
        maxPriorityFeePerGas: BigNumber.from('123'),
      }
      mockPopulateTransaction.mockResolvedValue(populatedRequest)

      // Act
      await service.prepareTransaction({ request })
